...
class Buffer(object):
  ...
  def __init__(self, max_size, total_seen=0):
    self.max_size = max_size
    self.data = []
    # Included this argument to hold the number of states from last time.
    self.total_seen = total_seen  # The number of items that passed through.
...
class _EndStats(collections.namedtuple(
    "EndStats", [
        "end_step",
        "end_time",
        "end_states",
        "end_trajectories", ])):
  """A stats class to hold relevant learner.jsonl data."""
  pass


def _config_from_json(json_dict):
  """Retrieve a Config from a dictionary obtained from config.json."""
  config = Config(
      game=json_dict["game"],
      path=json_dict["path"],
      learning_rate=json_dict["learning_rate"],
      weight_decay=json_dict["weight_decay"],
      train_batch_size=json_dict["train_batch_size"],
      replay_buffer_size=json_dict["replay_buffer_size"],
      replay_buffer_reuse=json_dict["replay_buffer_reuse"],
      max_steps=json_dict["max_steps"],
      checkpoint_freq=json_dict["checkpoint_freq"],

      actors=json_dict["actors"],
      evaluators=json_dict["evaluators"],
      uct_c=json_dict["uct_c"],
      max_simulations=json_dict["max_simulations"],
      policy_alpha=json_dict["policy_alpha"],
      policy_epsilon=json_dict["policy_epsilon"],
      temperature=json_dict["temperature"],
      temperature_drop=json_dict["temperature_drop"],
      evaluation_window=json_dict["evaluation_window"],
      eval_levels=json_dict["eval_levels"],

      nn_model=json_dict["nn_model"],
      nn_width=json_dict["nn_width"],
      nn_depth=json_dict["nn_depth"],
      observation_shape=json_dict["observation_shape"],
      output_size=json_dict["output_size"],

      quiet=json_dict["quiet"],
  )
  return config


def _end_stats_from_json(last_step_dict):
  """Retrieve an EndStats from a dictionary of a step of learner.jsonl."""
  end_stats = _EndStats(
      end_step=last_step_dict["step"],
      end_time=last_step_dict["time_rel"],
      end_states=last_step_dict["total_states"],
      end_trajectories=last_step_dict["total_trajectories"],
  )
  return end_stats


def _init_model_from_checkpoint(checkpoint, path):
  """Loads a model from a particular checkpoint in the specified path."""
  return model_lib.Model.from_checkpoint(
      os.path.join(path, "checkpoint-{}".format(checkpoint)))
...
@watcher
def actor(*, config, game, logger, checkpoint, queue):
...
@watcher
def learner(*, game, config, actors,
            evaluators, broadcast_fn, logger,
            end_stats, checkpoint):
  """A learner that consumes the replay buffer and trains the network."""
  if checkpoint is not None and end_stats is not None:
    replay_buffer = Buffer(config.replay_buffer_size,
                           total_seen=end_stats.end_states)
    model = _init_model_from_checkpoint(checkpoint, config.path)
    save_path = model.save_checkpoint(-1)
    total_trajectories = end_stats.end_trajectories
    start_step = end_stats.end_step + 1
    data_log = data_logger.DataLoggerJsonLines(path=config.path,
                                               name="learner",
                                               initial_time=end_stats.end_time,
                                               flush=True)
  else:
    replay_buffer = Buffer(config.replay_buffer_size)
    model = _init_model_from_config(config)
...
def _start_learning(config, end_stats=None, checkpoint=None):
  ...
  try:
    learner(game=game, config=config, actors=actors,  # pylint: disable=missing-kwoa
            ...
            end_stats=end_stats, checkpoint=checkpoint)
...
def _delete_newer_checkpoints(path, step):
  if step == -1:
    return
  ...
  for checkpoint_file in checkpoint_files:
    dot_position = checkpoint_file.find('.')
    checkpoint_number = int(checkpoint_file[len(prefix):dot_position])

    if checkpoint_number > step:
...
def from_checkpoint(path, checkpoint, steps):
  ...
  _start_learning(config, end_stats=end_stats, checkpoint=checkpoint)
